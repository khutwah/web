...
import type { Page, AyahLocationInfo } from '@/models/mushaf'

export { pages, parts, surahs, lajnah }
...
export function getAyahLocationInfo(
  ...
  const delta = lajnahPartIndex + (distanceToJuzEnd.page > 0 ? 1 : 0)
  const lajnahTotalJuz = foundLajnah.id - delta
  return {
    juz: foundJuz.id,
